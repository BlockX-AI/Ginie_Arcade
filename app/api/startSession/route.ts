...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isDbAvailable()) {
      // No database configured – use in-memory session store (local dev)
      memSessions.set(sessionId, { id: sessionId, walletAddress: wallet, gameId, nonce, startedAt, valid: true });
      return NextResponse.json({ sessionId, nonce, startedAt: startedAt.toISOString() });
    }
